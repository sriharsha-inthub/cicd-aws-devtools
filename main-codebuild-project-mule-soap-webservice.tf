...
################ CODEBUILD #######################
resource "aws_codebuild_project" "mule-soap-webservice" {
  count = "${var.create_mule-soap-webservice}" ? 1 : 0
  name          	= "mule-soap-webservice"
  description  		= "Mule application soap webservice"
  ...
  source {
    type     			= "GITHUB"
    location 			= "https://github.com/sriharsha-inthub/mule-soap-webservice.git"
    git_clone_depth 	= 1
  ...
  logs_config {
    cloudwatch_logs {
      group_name 	= "mule-soap-webservice"
      stream_name 	= "mule-soap-webservice"
    }
    s3_logs {
      status 				= "ENABLED"
...
output "badge_url-mule-soap-webservice" {
	value       =  "${join(",",aws_codebuild_project.mule-soap-webservice.*.badge_url)}"
}